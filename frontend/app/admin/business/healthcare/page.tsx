      ...
      <div className="flex gap-2 items-start flex-wrap">
        ...
        <label
          className={`px-3 py-2 text-sm rounded-md cursor-pointer transition-colors whitespace-nowrap ${
            uploading ? 'bg-gray-300 text-gray-500' : 'btn-primary'
          }`}
        >
...
function IdeasTab({ data, onChange }: { data: any; onChange: (d: any) => void }) {
  ...
  const addItem = () =>
    onChange({
      ...
      items: [
        ...items,
        { title: '', description: '', image: '', link: '', linkText: 'Read more', primaryButtonBg: '', primaryButtonTextColor: '', icon: 'Healthcare', stats: '10 mins read', statsLabel: '' },
      ],
    });
    ...
    <div className="space-y-5">
      ...
      <div className="border-t pt-5">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={addItem} className="text-xs px-3 py-1.5 btn-primary rounded-md hover:bg-secondary">
            + Add idea
          </button>
        </div>
        <div className="space-y-4">
