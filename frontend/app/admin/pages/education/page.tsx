...
const PAGE_SLUG = 'business-education';
...
const DEFAULT_SECTIONS: Record<string, any> = {
  ...
  'text-highlight': {
    ...
    description: 'Our ergonomically designed education furniture supports students in their learning journey and enables an effective, inspiring environment for educators and staff.',
    image: '', items: [],
  },
  placeholder: {
  ...
  'just-arrived': {
    ...
    title: 'Just arrived',
    description: 'for-business/education',
    link: '/products', linkText: 'View all products',
    image: '', items: [],
  },
  'ideas-section': {
    sectionId: 'ideas-section', type: 'cards', bgColor: 'white', order: 4, isVisible: true,
    title: 'Ideas for educational spaces',
    link: '/industries', linkText: 'Explore all',
    image: '', description: '',
    items: [
      {
        title: 'Building Future-Ready Classrooms',
        description: 'Modern educational institutions demand flexible, ergonomic spaces that adapt to diverse teaching methods.',
        image: '',
        link: '/industries',
        linkText: 'Read more',
        icon: 'Education',
...
function IdeaItemEditor({ item, idx, onChange, onRemove, onMoveUp, onMoveDown, isFirst, isLast }: { item: any; idx: number; onChange: (updated: any) => void; onRemove: () => void; onMoveUp: () => void; onMoveDown: () => void; isFirst: boolean; isLast: boolean }) {
  ...
  return (
    <div className="border border-gray-200 rounded-lg p-4 bg-gray-50 space-y-3">
      ...
      <Field label="Title" value={item.title || ''} onChange={(v) => set('title', v)} placeholder="e.g. Building Future-Ready Classrooms" />
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Category label" value={item.icon || ''} onChange={(v) => set('icon', v)} placeholder="e.g. Education" />
        <Field label="Read time" value={item.stats || ''} onChange={(v) => set('stats', v)} placeholder="e.g. 8 mins read" />
      </div>
      ...
      <ImageUploader label="Article image" value={item.image || ''} onChange={(v) => set('image', v)} folder="education" />
    </div>
  );
}
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const file of files) {
        const result = await uploadSingleImage(file, 'education');
...
function HeroTab({ data, onChange }: { data: any; onChange: (d: any) => void }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <Field label="Headline" value={data.title || ''} onChange={(v) => set('title', v)} placeholder="Designed for Learning Environments" />
      <Field label="Sub-headline" value={data.subtitle || ''} onChange={(v) => set('subtitle', v)} placeholder="Optional tagline below headline" />
      <Field label="Description text" value={data.description || ''} onChange={(v) => set('description', v)} textarea placeholder="One or two sentences about education furniture..." />
...
function HighlightTextTab({ data, onChange }: { data: any; onChange: (d: any) => void }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <Field label="Highlight text" value={data.description || ''} onChange={(v) => set('description', v)} textarea placeholder="Our ergonomically designed education furniture supports students..." hint="Keep it to 1–3 sentences. Displayed in large coral text." />
      <VisibilityToggle value={data.isVisible ?? true} onChange={(v) => set('isVisible', v)} />
    </div>
  );
}
    ...
    <div className="space-y-5">
      ...
      {data.isVisible && (
        <>
          ...
          <ImageUploader label="Image (optional)" value={data.image || ''} onChange={(v) => set('image', v)} folder="education" />
    ...
    <div className="space-y-5">
      ...
      <div className="border border-gray-200 rounded-lg p-4 space-y-3">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">Used only when no pinned products are set. Enter the category slug path (e.g. <code className="bg-gray-100 px-1 rounded">for-business/education</code>).</p>
        </div>
        <Field label="Category path" value={data.description || ''} onChange={(v) => set('description', v)} placeholder="for-business/education" />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
...
function IdeasTab({ data, onChange }: { data: any; onChange: (d: any) => void }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Field label="Section heading" value={data.title || ''} onChange={(v) => set('title', v)} placeholder="Ideas for educational spaces" />
      ...
      <div className="bg-gray-50 rounded-lg p-5 flex items-start gap-4">
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500 mt-1">
            ...
            <code className="text-xs bg-gray-200 px-1.5 py-0.5 rounded">/business/education</code>.
          </p>
        </div>
    ...
    try {
      ...
      try {
        ...
        if (err?.response?.status === 404) {
          await api.post('/pages', { slug: PAGE_SLUG, title: PAGE_TITLE, description: 'Education furniture solutions page', isPublished, sections });
        } else { throw err; }
      }
      setSaved(true);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-bold">Education Page</h1>
            <p className="text-gray-500 mt-1 text-sm">
              Manages content at{' '}
              <Link href="/business/education" target="_blank" className="text-accent underline underline-offset-2">
                /business/education ↗
              </Link>
            </p>
          </div>
