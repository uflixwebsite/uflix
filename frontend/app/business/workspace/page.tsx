...
// This file was moved from /category/workspace to /business/workspace
// Content derived from the Healthcare template and adapted for Workspace

const PH_HIGHLIGHT_TEXT =
  'Flexible work solutions and ergonomic systems built to improve productivity and wellbeing.';
...
function WorkspaceHero({ section }: { section?: Section }) {
  const allSlides = [
    {
      image: section?.image || '',
      title: section?.title || '',
            ...
            <article key={i} className="grid grid-cols-1 md:grid-cols-2" style={{ minHeight: 500 }}>
              ...
              <div className={`flex items-center bg-gray-50 px-8 sm:px-12 md:px-16 lg:px-20 py-14 ${imageLeft ? 'md:order-2' : 'md:order-1'}`}>
                <div className="max-w-lg w-full">
                  ...
                  {idea.link && (
                    <Link href={idea.link} className="inline-block px-7 py-3 rounded-full text-sm font-semibold transition-colors bg-white/10 hover:bg-white/20 text-white" style={{ backgroundColor: idea.primaryButtonBg || undefined, color: idea.primaryButtonTextColor || undefined }}>{idea.linkText || 'Read more'}</Link>
                  )}
                </div>
              </div>
            </article>
...
export default function WorkspacePage() {
  ...
  const ideasItems: any[] = (() => {
    const cmsItems = ideasSection?.items?.length
      ? ideasSection.items.map((it: any, idx: number) => ({
          category: it.icon || 'Workspace',
          readTime: it.stats || '7 mins read',
          title: it.title || '',
  ...
  const ideasHeading = ideasSection?.title || 'Ideas for workplace spaces';
