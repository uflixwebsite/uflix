...
// ─── Placeholder / sample data (shown until admin sets real content) ─────────

const PH_HIGHLIGHT_TEXT =
  'Our ergonomically designed healthcare products support patients in their healing journey and enable an efficient workflow for doctors and caregivers.';
      ...
      <div className="relative z-10 text-center px-4 pt-32 pb-16 max-w-5xl mx-auto">
        ...
        {title ? (
          ...
          <h1 className="text-5xl md:text-7xl font-bold text-white mb-6 leading-tight">
            Designed for <br />
            <span style={{ color: '#E87059' }}>Healing Environments</span>
          </h1>
        )}
...
function ProductSliderCard({ image, name, variant, link }: ProductSliderCardProps) {
  return (
...
// ─── Ideas / Articles — staggered full-bleed layout ─────────────────────────
//
//  Even rows:  [ IMAGE ─ left half ] [ text ─ right half ]
//  Odd rows:   [ text ─ left half  ] [ IMAGE ─ right half ]
//
//  With zero vertical gap between rows the inner corner of image[i] sits
//  exactly at the same point as the inner corner of image[i+1], producing
//  the "touching corners / staircase" effect.
...
function IdeasSection({ heading, exploreLink, exploreText, ideas }: { heading: string; exploreLink: string; exploreText: string; ideas: IdeaItem[] }) {
  ...
  return (
    <section className="bg-white">
      ...
      {/* Staggered full-bleed rows — no gap so corners touch */}
        ...
        {ideas.map((idea, i) => {
          ...
          return (
            <article
              ...
              style={{ minHeight: 500 }}
            >
              {/* ── Image column — no rounding, bleeds to outer edge ── */}
              <div
                className={`relative min-h-72 md:min-h-full ${
              ...
              {/* ── Text column ── */}
              <div
                className={`flex items-center bg-gray-50 px-8 sm:px-12 md:px-16 lg:px-20 py-14 ${
              ...
              >
                <div className="max-w-lg w-full">
                  ...
                  {idea.link && (
                    <Link
                      href={idea.link}
                      className="inline-block bg-gray-900 hover:bg-accent text-white px-7 py-3 rounded-full text-sm font-semibold transition-colors"
                    >
      ...
      {/* Bottom spacer so the last row doesn't clip into the footer */}
      <div className="pb-16" />
    </section>
  );
}
...
function PlaceholderSection({ section }: { section?: Section }) {
  ...
  // Renders whatever the admin configures for the placeholder section
  // If the section has a title + description + image it renders a simple content block
...
export default function HealthcarePage() {
  ...
  const fetchProducts = async (slugs: string[]) => {
    setProductsLoading(true);
    try {
      // Try resolving the category path
      ...
      if (node?._id) {
        ...
        // Fallback: fetch all business products
